...
mod tests {
    ...
    #[test]
    fn select_executes_only_json_branch_for_json_request() {
        let req = SilcrowRequest {
            is_silcrow: true,
            accepts_html: false,
            accepts_json: true,
        ...
        assert_eq!(response.status(), StatusCode::OK);
        assert_eq!(html_calls.load(Ordering::SeqCst), 0);
        assert_eq!(json_calls.load(Ordering::SeqCst), 1);
    }

    #[tokio::test]
    async fn select_406_includes_expected_message() {
        let req = SilcrowRequest {
            is_silcrow: false,
            accepts_html: true,
            accepts_json: false,
        ...
        assert_eq!(
            String::from_utf8(body.to_vec()).expect("fallback should be utf8"),
            "HTML not provided"
        );
    }
