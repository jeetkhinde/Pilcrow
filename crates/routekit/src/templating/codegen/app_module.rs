...
pub fn render_generated_app_module(
    ...
    for entry in page_entries {
        ...
        let any_load_returns_result = page_load.map_or(false, |s| s.returns_result)
        ...
        if is_streaming {
            if isr_opts.map_or(false, |o| o.is_active()) {
                return Err(route_config_error(
                    ...
                    "Use Deferred<T> for individual field streaming with ISR caching, or remove REVALIDATE for full-page streaming.",
                ));
            }
            ...
            if !deferred_fields.is_empty() || !deferred_html_fields.is_empty() {
                return Err(route_config_error(
                    entry,
                    "STREAMING = true cannot be combined with Deferred<T> or DeferredHtml fields.",
                    "Use one mechanism: either STREAMING = true for full-page streaming or explicit Deferred<T>/DeferredHtml fields.",
                ));
            }
            if page_load.map_or(false, |s| s.wants_client) {
                return Err(route_config_error(
                    ...
                    "Use Deferred<T> for client-based data fetching, or remove the PilcrowClient argument from the streaming page load().",
                ));
            }
        }
