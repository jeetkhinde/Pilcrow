...
///
/// Merges all field streams and emits `patch` events targeting the auto-generated live section.
...
pub fn __live_props_response(
    ...
    sse_stream(move |emit| async move {
        while let Some((field, json_str)) = merged.next().await {
            ...
            emit.send(SilcrowEvent::patch(serde_json::Value::Object(map), target))
                .await?;
        }
        Ok(())
    })
...
pub enum LiveTarget {
    /// Patch DOM bindings only (`:text`, `:class`, etc.). Default.
    #[default]
    Dom,
